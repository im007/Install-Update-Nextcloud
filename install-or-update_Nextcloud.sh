...
# Extract the version number and remove the "v" prefix
LATEST_VERSION=$(echo "$LATEST_RELEASE_JSON" | jq -r '.tag_name' | sed 's/^v//')

# Define location for the Nextcloud AppImage
NEXTCLOUD_LOC="/home/$USER/.local/share/applications"
APPIMAGE_PATH="$NEXTCLOUD_LOC/Nextcloud.AppImage"

# Check if the AppImage exists, extract the version number, and compare it with the latest version
...
if [ "$INSTALL_UPDATE" = true ]; then
    # Build the URL for the AppImage file, download it & save as Nextcloud.AppImage
